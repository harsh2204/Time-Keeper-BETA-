...
timeKeeper.controller('dayCtrl',['$routeParams','$http', '$scope', function($routeParams, $http, $scope) {
  ...
  var date = timetable.title.split(':')[1];
  date = date.split(' - ')[0];
  date = new Date(date.split('/')[2],Number(date.split('/')[1])-1,date.split('/')[0]);
  date.setDate(date.getDate()+(parseInt($routeParams.dayName)-1));
  this.day = timetable.schedule[parseInt($routeParams.dayName)-1].day+" - "+ date.toLocaleDateString();
  this.courses = timetable.schedule[parseInt($routeParams.dayName)-1].courses;
